...
def load_and_process_data(logs_folder, environment, plot_type, entropy_coefficient, seeds_to_plot, window_size, max_points=None):
    ...
    for file in files:
        ...
        # Note: coinrun uses progress-rank001.csv instead of progress.csv
        file_path = os.path.join(logs_folder, file, "progress-rank001.csv")
        if not os.path.exists(file_path):
            continue
            
        try:
            df = pd.read_csv(file_path)
            # Note: coinrun uses 'eprewmean' instead of 'rollout/ep_rew_mean'
            if 'eprewmean' not in df.columns:
                continue
                
            rewards = df['eprewmean']
...
def get_mean_run_for_coeff_type(all_files, target_entropy_coeff, target_plot_type, environment, seeds_to_plot, window_size):
    ...
    for file_dir in all_files:
        ...
        # Note: coinrun uses progress-rank001.csv instead of progress.csv
        file_path = os.path.join(LOGS_FOLDER, file_dir, "progress-rank001.csv")
        if not os.path.exists(file_path):
            continue
        ...
        try:
            df = pd.read_csv(file_path)
            # Note: coinrun uses 'eprewmean' instead of 'rollout/ep_rew_mean'
            if 'eprewmean' not in df.columns or df.empty:
                continue
...
def create_three_subplot_figure():
    ...
    for plot_type, ax, title in plot_configs:
        ...
        for entropy_coeff in available_coeffs:
            ...
            if x_vals is not None:
                ...
                x_vals = x_vals * 65536  # Timestep multiplier for coinrun
        ...
        ax.set_xlim(0, 700*65536)
        ax.set_ylim(3, 8)
        ax.set_xticks(np.arange(0, 700*65536, 1e7))
    ...
    for plot_type_to_aggregate in ["complexity", "entropy"]:
        ...
        x_values = np.arange(common_min_len_for_plot_type) * 65536  # Timestep multiplier for coinrun
    ...
    ax3.set_xlim(0, 700*65536)
    ax3.set_ylim(3, 8)
    ax3.set_xticks(np.arange(0, 700*65536, 1e7))
    ...
    # Save figure
    plot_filename = f"figures/three_subplot_coinrun_{ENVIRONMENT}.pdf"
