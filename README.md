...
For example, to train a CR-PPO agent on `CartPole-v1` you can run:
...
You can play with the `--entropy_value` parameter, which corresponds to the scaling factor for either entropy or complexity regularization. To switch from CR-PPO to PPO, just pass the argument `--only_entropy`., you can use the Dockerfile `Dockerfile.procgen` to build a Docker container in which running the code. Alternatively, you can set up a virtual environment with Python >=3.6,<=3.8 by executing:
...
and play with the `--entropy_value` parameter, which corresponds to the scaling factor for either entropy or complexity regularization. To rollback from CR-PPO to PPO, just pass the argument `--only_entropy`.
...
For example, to train a CR-PPO agent on `CARTerpillar` with 2 poles and default gravity you can run:

```
python3 main_CARTerpillar.py --n_carts 2 --entropy_value 1e-1 --seed 0 --timesteps 100000
```

You can play with the `--entropy_value` parameter, which corresponds to the scaling factor for either entropy or complexity regularization. To switch from CR-PPO to PPO, just pass the argument `--only_entropy`. The `--n_carts` and `--gravity` arguments allow you to change the difficulty of the environment.
